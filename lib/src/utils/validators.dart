...
var requiredValidator = RequiredValidator(errorText: trans().thisFieldIsRequired);
...
var passwordValidator = RequiredValidator(errorText: trans().passwordFieldIsRequired);

var newPasswordValidator = RequiredValidator(errorText: trans().newPasswordFieldIsRequired);
...
String? passwordConfirmationValidator(String value, String value2) => MatchValidator(
...
numericValidator({bool isRequired = true}) => MultiValidator([
  if(isRequired)
    RequiredValidator(errorText: trans().thisFieldIsRequired),
  PatternValidator(r'^\d+(\.\d+)?$', errorText: trans().thisFieldAcceptsDigitsOnly)
]);
